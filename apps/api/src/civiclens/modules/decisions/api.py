from __future__ import annotations

from typing import Annotated
from uuid import UUID

from fastapi import APIRouter, Depends, Header, HTTPException, status

from civiclens.modules.decisions.schemas import DecisionCreateRequest, HumanDecision
from civiclens.modules.decisions.service import (
    HumanDecisionPort,
    authenticate_demo_session,
    get_human_decision_service,
)
from civiclens.shared.errors import ConflictError, NotFoundError

router = APIRouter(prefix="/officer/needs", tags=["human-decisions"])
Service = Annotated[HumanDecisionPort, Depends(get_human_decision_service)]
SessionId = Annotated[UUID, Header(alias="X-Demo-Session-ID")]
Authorization = Annotated[str, Header(alias="Authorization", min_length=39, max_length=256)]
IdempotencyKey = Annotated[str, Header(alias="Idempotency-Key", min_length=16, max_length=128)]


@router.post(
    "/{need_id}/decisions",
    response_model=HumanDecision,
    status_code=status.HTTP_201_CREATED,
)
async def record_decision(
    need_id: UUID,
    request: DecisionCreateRequest,
    service: Service,
    session_id: SessionId,
    authorization: Authorization,
    idempotency_key: IdempotencyKey,
) -> HumanDecision:
    try:
        session = authenticate_demo_session(session_id, authorization)
        return service.append(
            session=session,
            need_id=need_id,
            request=request,
            idempotency_key=idempotency_key,
        )
    except NotFoundError as exc:
        raise HTTPException(status_code=404, detail={"code": exc.code}) from exc
    except ConflictError as exc:
        raise HTTPException(
            status_code=409,
            detail={"code": exc.code, "message": str(exc)},
        ) from exc


@router.get("/{need_id}/decisions", response_model=list[HumanDecision])
async def decision_history(
    need_id: UUID,
    service: Service,
    session_id: SessionId,
    authorization: Authorization,
) -> list[HumanDecision]:
    try:
        session = authenticate_demo_session(session_id, authorization)
        return service.history(session=session, need_id=need_id)
    except NotFoundError as exc:
        raise HTTPException(status_code=404, detail={"code": exc.code}) from exc
